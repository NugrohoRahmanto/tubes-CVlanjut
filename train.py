...
def validate_runtime_args(args):
    args.train_path = normalize_paths(args.train_path)
    args.eval_path = normalize_paths(args.eval_path)

    if not args.train_path:
        raise ValueError("No --train-path values were provided.")
    if not args.eval_path:
        raise ValueError("No --eval-path values were provided.")

    missing_train = [path for path in args.train_path if not os.path.isdir(path)]
    missing_eval = [path for path in args.eval_path if not os.path.isdir(path)]
    if missing_train:
        raise FileNotFoundError("Training dataset folders not found:\n" + "\n".join(missing_train))
    if missing_eval:
        raise FileNotFoundError("Evaluation dataset folders not found:\n" + "\n".join(missing_eval))

    if args.fine_tune and not os.path.isfile(args.restore_from):
        raise FileNotFoundError(f"Checkpoint not found: {args.restore_from}")
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is not available. DocSAM training currently requires CUDA and torchrun/NCCL.")
    return args
