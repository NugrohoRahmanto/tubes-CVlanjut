...
class Mask2FormerPixelLevelModule(nn.Module):
    def __init__(self, config: Mask2FormerConfig):
        ...
        self.fpn = torchvision.ops.FeaturePyramidNetwork(planes, config.feature_size)
    ...
    def forward(self, pixel_values: Tensor, image_bboxes=None, output_hidden_states: bool = False) -> Mask2FormerPixelLevelModuleOutput:
        ...
        # dsize = (backbone_features[0].size(2), backbone_features[0].size(3))
        # [x1, x2, x3, x4] = backbone_features
        # feats = {"x1":x1.contiguous(), "x2":x2.contiguous(), "x3":x3.contiguous(), "x4":x4.contiguous()}
        # feats = self.fpn(feats) 
        # [x1, x2, x3, x4] = [feats["x1"], feats["x2"], feats["x3"], feats["x4"]]
        # feat_fpn = self.feat_merge([x1, x2, x3, x4], dsize=dsize)
        
        # output = Mask2FormerPixelLevelModuleOutput(
        #     encoder_hidden_states=tuple(backbone_features),
        #     decoder_hidden_states=(x4, x3, x2, x1),
        #     decoder_mask_features=feat_fpn,
        # )
...
class TransformerDecoderLayer(nn.TransformerDecoderLayer):
    ...
    def forward(
        ...
        x = tgt
        if self.norm_first:
            x = x + self._sa_block(self.norm1(x), tgt_pos, tgt_mask, tgt_key_padding_mask, tgt_is_causal)
            ...
            x = self.norm1(x + self._sa_block(x, tgt_pos, tgt_mask, tgt_key_padding_mask, tgt_is_causal))
...
class Mask2FormerMaskedAttentionDecoderLayer(nn.Module):
    ...
    def __init__(self, config: Mask2FormerConfig, is_multi_scale=False):
        ...
        self.instance_pos_fc1 = nn.Linear(query_hidden_dim, image_hidden_dim)
        ...
        self.instance_pos_fc2 = nn.Linear(image_hidden_dim, query_hidden_dim)
        
        # Semantic-level feature processing layers
        self.semantic_fc1 = nn.Linear(query_hidden_dim, image_hidden_dim)
        self.semantic_pos_fc1 = nn.Linear(query_hidden_dim, image_hidden_dim)
        ...
        self.semantic_pos_fc2 = nn.Linear(image_hidden_dim, query_hidden_dim)
    ...
    def forward(
        ...
        pixel_posembs: torch.Tensor = None,
        encoder_posembs: List[torch.Tensor] = None,
        pixel_padding_masks: List[torch.Tensor] = None,
        instance_states: torch.Tensor = None,
        semantic_states: torch.Tensor = None,
        instance_posembs: torch.Tensor = None,
        semantic_posembs: torch.Tensor = None,
        instance_attentions: torch.Tensor = None,
        semantic_attentions: torch.Tensor = None,
        instance_padding_masks: torch.Tensor = None,
        semantic_padding_masks: torch.Tensor = None,
    ):
        """
        Forward pass of the model.
        
        Args:
            pixel_embeddings (torch.Tensor): Embeddings from the pixel level.
            encoder_hidden_states (List[torch.Tensor]): Hidden states from the encoder.
            pixel_posembs (torch.Tensor): Position embeddings for pixels.
            encoder_posembs (List[torch.Tensor]): Position embeddings from the encoder.
            pixel_padding_masks (List[torch.Tensor]): Padding masks for pixels.
            instance_states (torch.Tensor): Initial instance query states.
            semantic_states (torch.Tensor): Initial semantic query states.
            instance_posembs (torch.Tensor): Position embeddings for instance queries.
            semantic_posembs (torch.Tensor): Position embeddings for semantic queries.
            instance_attentions (torch.Tensor): Attention masks for instance queries.
            semantic_attentions (torch.Tensor): Attention masks for semantic queries.
            instance_padding_masks (torch.Tensor): Padding masks for instance queries.
            semantic_padding_masks (torch.Tensor): Padding masks for semantic queries.
            
        Returns:
            [instance_states, semantic_states]: Updated instance and semantic states after processing.
        """
        
        # Query Fusion Step 1
        hidden_states = torch.cat((instance_states, semantic_states), dim=1)
        hidden_posembs = torch.cat((instance_posembs, semantic_posembs), dim=1)
        padding_masks = torch.cat((instance_padding_masks, semantic_padding_masks), dim=1)
        hidden_states = self.query_fusion_encoder_1(hidden_states, src_pos=hidden_posembs, src_mask=None, src_key_padding_mask=padding_masks)
        instance_states = hidden_states[:, :instance_states.size(1), :]
        semantic_states = hidden_states[:, instance_states.size(1):, :]
    
        # Instance Query Processing
        if self.is_multi_scale:
            instance_states = self.instance_fc1(instance_states)
            instance_posembs = self.instance_pos_fc1(instance_posembs)
            for level in range(self.num_levels):
                instance_states = self.instance_decoder[level](
                    tgt=instance_states,
                    memory=encoder_hidden_states[level].flatten(2).transpose(1,2),
                    tgt_pos=instance_posembs,
                    memory_pos=encoder_posembs[level].flatten(2).transpose(1,2),
                    tgt_mask=None,
            ...
            instance_posembs = self.instance_pos_fc2(instance_posembs)
        else:
            instance_states = self.instance_fc1(instance_states)
            instance_posembs = self.instance_pos_fc1(instance_posembs)
            instance_states = self.instance_decoder(
                tgt=instance_states,
                memory=pixel_embeddings.flatten(2).transpose(1,2),
                tgt_pos=instance_posembs,
                memory_pos=pixel_posembs.flatten(2).transpose(1,2),
                tgt_mask=atten_masks,
        ...
        # Semantic Query Processin
        if self.is_multi_scale:
            semantic_states = self.semantic_fc1(semantic_states)
            semantic_posembs = self.semantic_pos_fc1(semantic_posembs)
            for level in range(self.num_levels):
                semantic_states = self.semantic_decoder[level](
                    tgt=semantic_states,
                    memory=encoder_hidden_states[level].flatten(2).transpose(1,2),
                    tgt_pos=semantic_posembs,
                    memory_pos=encoder_posembs[level].flatten(2).transpose(1,2),
            ...
            semantic_posembs = self.semantic_pos_fc2(semantic_posembs)
        else:
            semantic_states = self.semantic_fc1(semantic_states)
            semantic_posembs = self.semantic_pos_fc1(semantic_posembs)
            semantic_states = self.semantic_decoder(
                tgt=semantic_states,
                memory=pixel_embeddings.flatten(2).transpose(1,2),
                tgt_pos=semantic_posembs,
                memory_pos=pixel_posembs.flatten(2).transpose(1,2),
        ...
        hidden_posembs = torch.cat((instance_posembs, semantic_posembs), dim=1)
        padding_masks = torch.cat((instance_padding_masks, semantic_padding_masks), dim=1)
        hidden_states = self.query_fusion_encoder_2(hidden_states, src_pos=hidden_posembs, src_mask=None, src_key_padding_mask=padding_masks)
        instance_states = hidden_states[:, :instance_states.size(1), :]
        semantic_states = hidden_states[:, instance_states.size(1):, :]
...
class Mask2FormerMaskedAttentionDecoder(nn.Module):
    ...
    def __init__(self, config: Mask2FormerConfig):
        ...
        self.image_position_conv = conv_bn_relu(mask_feature_dim, image_hidden_dim, kernel_size=1, is_bn=False, is_relu=False)
        self.mask_position_conv  = conv_bn_relu(mask_feature_dim, mask_feature_dim, kernel_size=1, is_bn=False, is_relu=False)
        ...
        # position embedding pooling
        self.posi_roi_size = (7, 7)
        self.instance_posi_feat_projection = nn.Sequential(
            linear_bn_relu(mask_feature_dim*self.posi_roi_size[0]*self.posi_roi_size[1], query_hidden_dim, is_bn=True, is_relu=True),
            linear_bn_relu(query_hidden_dim, query_hidden_dim, is_bn=True, is_relu=True),
            linear_bn_relu(query_hidden_dim, query_hidden_dim, is_bn=False, is_relu=False))
        
        self.semantic_posi_feat_projection = nn.Sequential(
            linear_bn_relu(mask_feature_dim, query_hidden_dim, is_bn=True, is_relu=True),
            linear_bn_relu(query_hidden_dim, query_hidden_dim, is_bn=True, is_relu=True),
            linear_bn_relu(query_hidden_dim, query_hidden_dim, is_bn=False, is_relu=False))


        # query decoding
        self.decoder_layers = nn.ModuleList([Mask2FormerMaskedAttentionDecoderLayer(config, self.is_multi_scale) for stage in range(self.num_stages)])
        ...
        for idx in range(self.num_stages + 1):
            ...
            if idx > 0:
                ...
                instance_states, semantic_states = self.decoder_layers[idx - 1](
                        ...
                        pixel_posembs = mask_posembs,
                        encoder_posembs = image_posembs,
                        pixel_padding_masks = pixel_padding_masks,
                        instance_states = instance_states,
                        semantic_states = semantic_states,
                        instance_posembs = instance_posembs,
                        semantic_posembs = semantic_posembs,
            ...
            # instance segmentation
            if idx > 0:
                instance_masks, instance_feats = self.instance_mask_predictor(instance_states + instance_posembs, pixel_embeddings + mask_posembs)

            # instance detection
            instance_bboxes = instance_bboxes + self.instance_bbox_predictor(instance_states + instance_posembs) * 100
