...
class DocSAM(nn.Module):
    ...
    def __init__(self, model_size="base"):
        ...
        if model_size=="base":
            mask2former_path = PROJECT_ROOT / "pretrained_model/mask2former/facebook-mask2former-swin-base-coco-panoptic"
        elif model_size=="large":
            mask2former_path = PROJECT_ROOT / "pretrained_model/mask2former/facebook-mask2former-swin-large-coco-panoptic"
        else:
            mask2former_path = PROJECT_ROOT / "pretrained_model/mask2former/facebook-mask2former-swin-base-coco-panoptic"
        print("mask2former_path:", str(mask2former_path))
        
        sentence_path = PROJECT_ROOT / "pretrained_model/sentence/all-MiniLM-L6-v2"
        print("sentence_path:", str(sentence_path))

        if not mask2former_path.is_dir():
            raise FileNotFoundError(f"Mask2Former pretrained folder not found: {mask2former_path}")
        if not sentence_path.is_dir():
            raise FileNotFoundError(f"Sentence pretrained folder not found: {sentence_path}")
        
        # Set your configurations here.
        self.config = Mask2FormerConfig.from_pretrained(str(mask2former_path))
        self.config.sentence_path = str(sentence_path)
        ...
        self.mask2former = Mask2FormerForUniversalSegmentation.from_pretrained(str(mask2former_path), config=self.config, ignore_mismatched_sizes=True)
